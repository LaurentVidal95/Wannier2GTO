...
function integral(X₁::GaussianPolynomial, X₂::GaussianPolynomial, args...; type=:overlap)
    if isempty(args) && type==:atomic
...
function integral(Φ₁::BasisFunction, Φ₂::BasisFunction, args...;
                  type=:overlap)
...
function integral(Wc₁::CompressedWannier, Wc₂::CompressedWannier, args...;
    ...
    for (λ₁, Φ₁) in zip(Wc₁.coefficients, Wc₁.basis_functions)
        for (λ₂, Φ₂) in zip(Wc₂.coefficients, Wc₂.basis_functions)
            output += λ₁*λ₂*integral(Φ₁, Φ₂, args...; type)
        end
    end
    output
end
