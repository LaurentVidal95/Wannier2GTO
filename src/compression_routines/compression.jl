...
"""
function compress_graphene_pz_wannier(basis_SC::PlaneWaveBasis, W_pz, α;
                                      ...
                                      Res_init = W_pz,  # initial residual
    ...
    w_proj = [zero(W_pz_k) for W_pz_k in W_pz]; Res = Res_init .- w_proj
    W_pz_norm = Hs_norm(basis, W_pz, s=s)
    ...
    # Compute blobs angle
    (iszero(θ)) && ( @show θ = find_blobs_angle(basis, W_pz, α) )
